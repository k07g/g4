...
func baseCognitoEnv() map[string]string {
	return map[string]string{
		...
		"DATABASE_URL":          "postgres://localhost/g4",
	}
}
...
func TestLoad_CognitoProviderSuccess(t *testing.T) {
	...
	if cfg.CognitoUserPoolID != "pool-id" || cfg.CognitoClientID != "client-id" {
		t.Errorf("unexpected Cognito config: %+v", cfg)
	}
	if cfg.DatabaseURL != "postgres://localhost/g4" {
		t.Errorf("DatabaseURL = %q, want %q", cfg.DatabaseURL, "postgres://localhost/g4")
	}
}
...
func TestLoad_MissingRequiredVars(t *testing.T) {
	tests := []struct {
		...
		{
			name: "missing COGNITO_USER_POOL_ID for cognito provider",
			env: func() map[string]string {
				e := baseCognitoEnv()
				e["COGNITO_USER_POOL_ID"] = ""
				return e
			}(),
			wantErr: "COGNITO_USER_POOL_ID",
		},
		{
			name: "missing COGNITO_CLIENT_ID for cognito provider",
			env: func() map[string]string {
				e := baseCognitoEnv()
				e["COGNITO_CLIENT_ID"] = ""
				return e
			}(),
			wantErr: "COGNITO_CLIENT_ID",
		},
		{
